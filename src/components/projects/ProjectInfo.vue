...
<template>
	<div class="block sm:flex gap-0 sm:gap-10 mt-14">
		...
		<div class="w-full sm:w-1/3 text-left">
			...
			<div class="mb-7">
				...
				<ul class="leading-loose">
					<li v-for="info in projectInfo.companyInfos" :key="info"
						...
						<a :href="info.title == 'Website' ?info.details :'#'" :class="info.title == 'Website' || info.title == 'Phone'
			...
			<div class="mb-7">
				<p class="font-general-medium text-2xl text-secondary-dark dark:text-secondary-light mb-2">
					{{ projectInfo.projectCredentialsTitle }}
				</p>
				<ul class="leading-loose">
					<li v-for="info in projectInfo.projectCredentialsInfo" :key="info"
						class="font-general-regular text-ternary-dark dark:text-ternary-light">
						
						<span>{{ info.title }}: </span>
						<a :href="info.title == 'Link' ? info.details :'#'" :class="info.title == 'Link' || info.title == 'Phone'
								? 'hover:underline cursor-pointer'
								: ''
							" aria-label="Project Website and Phone">{{ info.details }}</a>
					</li>
